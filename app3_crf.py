...
crf_img=crf(image_copy, pred,use_2d = True)

crf_img=np.array(crf_img,dtype=np.uint8)

print('crf_img.shape=',crf_img.shape)
print('***********************************')

colored_pred_crf=AddTextToMask(crf_img[:,:,0],target_names)

st.image(colored_pred_crf, caption='crf '+str(target)+'.png')
